<script lang="ts">
  ...
  function evidenceGroups(plays: Evidence[]) {
    const definitions = [
      {key: 'baseline', label: 'Baseline window'},
...
<div class="app-shell">
  ...
  <main>
      ...
      <section class="ask-panel">
        <div class="intro-grid">
          ...
          <details class="data-manager" class:nba-data-manager={activeSport === 'nba'} bind:open={dataManagerOpen}>
            <summary>
              <span><strong>Manage Local {activeSport === 'nba' ? 'SportsDataverse NBA' : 'nflverse'} Data</strong><small>Choose the seasons and packages to load for your investigations.</small></span><b>{workspaceLoading ? 'Loading data…' : `${datasets.length} local files`}
              <i>
        ...
        <section class="metric-card" aria-labelledby="metric-heading">
          ...
          <p class="section-help">
            ...
            Checked metrics are included and unchecked metrics are excluded. Use Recommended Metrics replaces the current selection with the recommended
            set.</p>
    ...
    {:else if active}
      ...
      <section class="plays">
        <div class="section-title"><span>REPRESENTATIVE {activeSport === 'nba' ? 'POSSESSIONS' : 'PLAYS'}</span><small>Balanced evidence from both
          windows</small>
        </div>
