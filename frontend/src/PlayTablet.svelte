<script lang="ts">
    ...
    $: startX = Math.max(10, Math.min(110, 10 + (100 - (play.visualization?.yardline_100 ?? 50))));
    $: lineToGainX = Math.max(10, Math.min(110, startX + (play.visualization?.yards_to_go ?? 0)));
    $: gain = Math.max(-15, Math.min(65, play.visualization?.yards_gained ?? play.visualization?.air_yards ?? 8));
    $: location = play.visualization?.pass_location ?? play.visualization?.run_location;
    ...
    function locationOffset(location?: string) {
        if (location?.toLowerCase() === 'left') return -11;
        if (location?.toLowerCase() === 'right') return 11;
        return 0;
    }
...
<section class="tablet" aria-label={`Play schematic for ${play.game_id}, play ${play.play_id}`}>
    ...
    <div class="field-wrap">
        <svg class="field" viewBox="0 0 120 53.3" role="img" aria-label="Schematic play direction">
            <defs>
                <marker id="play-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
                    <path d="M 0 0 L 10 5 L 0 10 z" fill={palette[0]}/>
                </marker>
            </defs>
            ...
            {/each}
            <line x1={startX} x2={startX} y1="2" y2="51.3" class="scrimmage"/>
            {#if play.visualization?.yards_to_go != null}
                <line x1={lineToGainX} x2={lineToGainX} y1="2" y2="51.3" class="line-to-gain"/>
            {/if}
            <path
                d={`M ${startX} 26.65 Q ${startX + gain * .55} ${26.65 + locationOffset(location) * .35}, ${Math.max(3, Math.min(117, startX + gain))} ${26.65 + locationOffset(location)}`}
                class="play-path" style={`stroke:${palette[0]}`} marker-end="url(#play-arrow)"/>
            <circle cx={startX} cy="26.65" r="1.5" fill={palette[1]} class="player-dot"/>
        </svg>
    </div>
    <div class="play-details">
        ...
        {#if offensePlayers.length || defensePlayers.length}
            <div class="on-field">
                <div class="on-field-heading"><strong>Recorded on-field personnel</strong><span>Participation package · positions are listed, not spatially inferred</span></div>
        ...
        {/if}
        <small>This schematic uses recorded play-by-play attributes and does not infer routes, assignments, or player locations.</small>
    </div>
</section>
